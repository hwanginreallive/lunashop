...
const getAllProductsDeny = () => products_deny;

const getProducts = (count) => {
    const start = Math.floor(Math.random() * count);
    return products_deny.slice(start, start + count);
};

const getProductDenyByID = (id) => products_deny.find((e) => e.id === id);

const getCartItemsInfo = (cartItems) => {
    let res = [];
    if (cartItems.length > 0) {
        cartItems.forEach((e) => {
            let product = getProductDenyByID(e.id);
            res.push({
                ...e,
                product: product,
            });
...
export default productDenyData;
