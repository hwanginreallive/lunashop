...
const getAllProductShiping = () => products_shipping;

const getProducts = (count) => {
    const start = Math.floor(Math.random() * count);
    return products_shipping.slice(start, start + count);
};

const getProductShipingByID = (id) => products_shipping.find((e) => e.id === id);

const getCartItemsInfo = (cartItems) => {
    let res = [];
    if (cartItems.length > 0) {
        cartItems.forEach((e) => {
            let product = getProductShipingByID(e.id);
            res.push({
                ...e,
                product: product,
            });
...
const productShipingData = {
    getAllProductShiping,
    getProducts,
    getProductShipingByID,
    getCartItemsInfo,
};

export default productShipingData;
